...
// Layer 1: Sources
//
// Pure data + logic, no rendering. Extracted from widgets/settings.sig
// so that platform/input.sig (Layer 1) can embed it in AppState without
// a cross-layer import.

const Config = @import("../config.sig").Config;
const ConfigStr = @import("../config.sig").ConfigStr;
const MAX_SUBS = @import("../config.sig").MAX_SUBS;
const SourceMetadata = @import("../metadata.sig").SourceMetadata;
