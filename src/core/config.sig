...
//
// Fixed-size config struct parsed from JSON using core/json.sig helpers.
...
const ct = @import("config_types.sig");

// Re-export types so callers can still use @import("config.sig").ConfigStr etc.
...
pub const Config = struct {
    ...
    subs: [MAX_SUBS]Subscription = @as([MAX_SUBS]Subscription, @splat(.{})),
    sub_count: usize = 0,
    ...
    creds: [SOURCE_COUNT]SourceCreds = @as([SOURCE_COUNT]SourceCreds, @splat(.{})),
