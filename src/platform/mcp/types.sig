...
pub const TOOLS = [_]ToolDef{
    ...
    // ── Strategy panel ──────────────────────────────────────────────
    .{ .name = "strategy", .desc =
        \\Strategy panel: author, backtest, tune, and arm. Payload goes in `args`. action: open | close | set_script (args.script) | set_lookback (args.window) | set_window (args.bars and/or args.offset in 1m bars, for walk-forward over earlier periods) | set_tf (args.tf seconds: 60=1m,300=5m,900=15m,3600=1h; 0=slot default) | run (backtest the display slot) | run_all (parallel backtest all cached slots) | walk_forward (native randomized walk-forward across all cached slots; args.windows, args.seed, args.win 1m-bars, args.max_offset 1m-bars, args.tf seconds — runs in-exe then read the aggregate stats JSON via get_all) | get_all (last batch OR walk-forward results JSON) | get_result (panel status + last result) | get_params (declared input() params JSON) | set_param (args.name + args.value) | evolve (native genetic strategy discovery; args.op: start|stop|get_status|get_champion; start args: generations, population, seed, windows, win 1m-bars, max_offset 1m-bars, tf, elite — runs a background GA that mutates+selects strategies scored honestly across a train/val/holdout time split; poll get_status / read the holdout-gated champion JSON via get_status) | toggle_arm (LIVE order execution safety gate).
    , .schema = SCHEMA_UNIFIED },
