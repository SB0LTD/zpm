...
//   - Relative file imports (starting with "./" or containing ".sig")
//
// Forbidden imports:
...
/// contains ".sig"), which is allowed for intra-module file references.
fn isRelativeImport(name: []const u8) bool {
    if (name.len >= 2 and name[0] == '.' and (name[1] == '/' or name[1] == '.')) return true;
    // Check for ".sig" anywhere in the name
    if (name.len >= 4) {
        var i: usize = 0;
        while (i + 4 <= name.len) : (i += 1) {
            if (std.mem.eql(u8, name[i..][0..4], ".sig")) return true;
        }
    }
    return false;
}
