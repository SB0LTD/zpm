...
/// Packet number spaces — local definition since conn.sig doesn't exist yet.
...
pub const AckResult = struct {
    acked_count: u16 = 0,
    lost_count: u16 = 0,
    acked: [64]u64 = @as([64]u64, @splat(0)),
    lost: [64]SentPacketInfo = @as([64]SentPacketInfo, @splat(.{})),
};
...
pub const RecoveryEngine = struct {
    ...
    pub fn init(qpc_freq: u64) RecoveryEngine {
        return .{
            ...
            .largest_sent_pkt = 0,
            .sent = [3][max_sent_packets]SentPacketInfo{
                @as([max_sent_packets]SentPacketInfo, @splat(.{})),
                @as([max_sent_packets]SentPacketInfo, @splat(.{})),
                @as([max_sent_packets]SentPacketInfo, @splat(.{})),
            },
