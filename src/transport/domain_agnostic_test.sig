...
//
// Scans all transport module source files EXCEPT appmap.sig for forbidden
...
// Only appmap.sig bridges zpm-specific operations to QUIC lanes.
//
...
// Every transport .sig file EXCEPT appmap.sig (the sole zpm-aware bridge).

const modules = .{
    .{ "udp.sig", @embedFile("udp.sig") },
    .{ "packet.sig", @embedFile("packet.sig") },
    .{ "crypto.sig", @embedFile("crypto.sig") },
    .{ "conn.sig", @embedFile("conn.sig") },
    .{ "recovery.sig", @embedFile("recovery.sig") },
    .{ "streams.sig", @embedFile("streams.sig") },
    .{ "datagram.sig", @embedFile("datagram.sig") },
